      ...
      {links.map((link) => (
        <NavLink key={link.to} to={link.to} end={link.end}>
          ...
          {({ isActive }) => (
            <motion.div
              whileHover={{ x: 2 }}
              className={`relative flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-colors ${isActive ? "bg-brass-light text-brass font-medium" : "text-text/60 hover:text-text hover:bg-stone/40"}`}
            >
              {" "}
