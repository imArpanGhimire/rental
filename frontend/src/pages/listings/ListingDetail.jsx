    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-ink/40 px-4 backdrop-blur-sm">
      <div className="relative w-full max-w-md rounded-2xl bg-bg p-6 shadow-2xl">
        <button
          type="button"
          onClick={onClose}
          className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full transition-colors hover:bg-ivory"
          aria-label="Close"
        >
          <X size={16} />
        </button>
        ...
        {sent ? (
          <div className="py-6 text-center">
            ...
            <button
              type="button"
              onClick={onClose}
              className="mt-5 rounded-full bg-ink px-6 py-2.5 text-sm font-medium text-ivory transition-opacity hover:opacity-90"
            >
              Close
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <label className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className="rounded-xl border border-stone bg-transparent px-3 py-2.5 text-sm outline-none focus:border-brass"
              />
            </label>
            ...
            <label className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                className="resize-none rounded-xl border border-stone bg-transparent px-3 py-2.5 text-sm outline-none focus:border-brass"
              />
            </label>
            ...
            <button
              ...
              className="mt-1 rounded-full bg-ink px-6 py-3 text-sm font-medium text-ivory transition-opacity hover:opacity-90 disabled:opacity-60"
            >
...
export default function ListingDetail() {
  ...
  const isRenterAccount = user?.role === "renter";

  function handleToggleSave() {
    if (!listing?._id) return;

    /*
     * Owners should never reach this function because
     * Save is hidden for them, but keep the guard anyway.
     */
    if (isOwnerAccount) {
      return;
    }
    ...
    /*
     * Only renters can save listings.
     */
    if (!isRenterAccount) {
      return;
    }
  ...
  const canReview = isRenterAccount && !isOwnerOfThis;
  ...
  return (
    <AppShell>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div>
              <div className="mb-2 flex flex-wrap items-center gap-2">
                ...
                {isAvailable ? (
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-semibold text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300">
                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                    Available
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-black/[0.045] px-2.5 py-1 text-[10px] font-semibold text-[#2b2d31]/55 dark:bg-white/[0.06] dark:text-white/45">
                    ...
                    return (
                      <div
                        ...
                        className="
                          ...
                          "
                      >
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-stone/60 bg-ivory/60 text-text/70">
            ...
            {description && (
              <div>
                ...
                <button
                  ...
                  className="mt-1 text-sm font-medium text-text underline underline-offset-2 transition-colors hover:text-brass"
                >
                  {expanded
            ...
            {owner && (
              <section className="overflow-hidden rounded-[22px] border border-black/[0.07] bg-white/45 dark:border-white/[0.07] dark:bg-white/[0.025]">
                <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex min-w-0 items-center gap-3.5">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full border border-black/[0.07] bg-[#ecebe7] dark:border-white/[0.08] dark:bg-white/[0.06]">
                      {ownerPhoto ? (
                        <img
                          src={ownerPhoto}
                          alt={ownerName}
                  ...
                  {/* Owners do not get renter contact action */}

                  {!isOwnerAccount && owner?.phone && (
                    <a
                      href={`tel:${owner.phone}`}
                      className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-full border border-black/[0.08] bg-white/60 px-4 text-[11px] font-semibold text-[#202226] no-underline transition-colors hover:bg-white dark:border-white/[0.08] dark:bg-white/[0.05] dark:text-white dark:hover:bg-white/[0.08]"
                    >
                ...
                {!isAvailable && (
                  <div className="border-t border-black/[0.06] bg-black/[0.025] px-5 py-3 text-[10px] leading-5 text-[#2b2d31]/48 dark:border-white/[0.06] dark:bg-white/[0.025] dark:text-white/40">
            ...
            <div className="flex flex-col gap-4 pt-1 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <div className="mt-2">
                  {isAvailable ? (
                    ...
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-black/[0.045] px-2.5 py-1 text-[10px] font-semibold text-[#2b2d31]/50 dark:bg-white/[0.06] dark:text-white/45">
                      <CircleOff size={11} />
                      Rented
                    </span>
                  )}
              ...
              {/* ============================================
                  ONLY RENTER / GUEST CAN SEE THESE ACTIONS.
                  OWNERS NEVER SEE SAVE.
              ============================================ */}
      ...
      {visitModalOpen && isAvailable && !isOwnerAccount && (
        <VisitRequestModal
          listing={listing}
